module ID_EX_reg (
    input  logic        i_clk,
    input  logic        i_reset,
    input  logic        i_stall, // Th??ng ID/EX ít khi Stall, ch? y?u là Flush
    input  logic        i_flush, // ??y bong bóng (NOP) vào khi có Hazard
    ...
    always_ff @(posedge i_clk or negedge i_reset) begin
        if (!i_reset || i_flush) begin
            // Khi Reset ho?c Flush (Hazard), t?t c? tín hi?u ?i?u khi?n v? 0 (NOP)
            o_mem_wren  <= 0;
            o_mem_rden  <= 0;
            o_reg_wren  <= 0;
            o_wb_sel    <= 0;
            ...
            o_br_un     <= 0;
            o_funct3    <= 0;
            o_pc_sel    <= 0; // Quan tr?ng: Không nh?y lung tung
            
            // D? li?u có th? gi? nguyên ho?c v? 0 (không quan tr?ng vì Control=NOP)
            o_pc        <= 0;
            o_rs1_data  <= 0;
            o_rs2_data  <= 0;
            o_imm       <= 0;
            ...
            // C?p nh?t bình th??ng khi không Stall
